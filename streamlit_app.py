...
system_resistancepps = system_resistance / PS_number

# Button to recalculate
if st.sidebar.button("Recalculate"):
    # Perform the recalculation
    power, voltage = recalculate(system_resistancepps, coil_current)
    
    # Display the results
    st.write(f"**Calculated Power per PS:** {power:.2f} kWatt")
    st.write(f"**Calculated Voltage per PS:** {voltage:.2f} V")
    st.write(f"**Resistance per PS:** {system_resistancepps:.2f} Ohm")
